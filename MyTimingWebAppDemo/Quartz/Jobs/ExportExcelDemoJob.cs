...
namespace MyTimingWebAppDemo.Quartz.Jobs
{
    internal class ExportExcelDemoJob : IJob
    {
        ...
        public Task Execute(IJobExecutionContext context)
        {
            try
            {
                const string reportTitle = "2013 年度五大公司实际情况与原计划的百分比";
                var file = new FileInfo($"{_exportFileAddress.ExportExcel}\\Excel图表测试.xlsx");
                ...
                var dataPercent = GetDataPercent();
                ...
                if (worksheet.Drawings.AddChart("ColumnStackedChart", eChartType.LineMarkers) is ExcelLineChart chart)
                {
                    ...
                    // 设置月份   
                    for (var col = 1; col <= dataPercent.Columns.Count; col++)
                        worksheet.Cells[1, col].Value = dataPercent.Columns[col - 1].ColumnName;

                    // 设置数据   
                    for (var row = 1; row <= dataPercent.Rows.Count; row++)
                    {
                        for (var col = 1; col <= dataPercent.Columns.Count; col++)
                        {
                            var strValue = dataPercent.Rows[row - 1][col - 1].ToString();
                            if (col == 1)
                            {
                            ...
                            {
                                var realValue = double.Parse(strValue!);
        ...
        private static DataTable GetDataPercent()
        {
            ...
            foreach (var monthName in MonthNames) data.Columns.Add(new DataColumn(monthName, typeof(double)));
