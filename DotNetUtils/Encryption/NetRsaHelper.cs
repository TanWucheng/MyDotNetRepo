...
namespace DotNetUtils.Encryption
{
    public class NetRsaHelper
    {
        ...
        {
            using var rsaCryptoServiceProvider = new RSACryptoServiceProvider(RsaKeySize);
            var publicKey = rsaCryptoServiceProvider.ToXmlString(false);
            var privateKey = rsaCryptoServiceProvider.ToXmlString(true);
