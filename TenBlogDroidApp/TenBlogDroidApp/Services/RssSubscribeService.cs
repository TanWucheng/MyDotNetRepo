...
namespace TenBlogDroidApp.Services
{
    internal static class RssSubscribeService
    {
        ...
        public static Task<List<BlogEntryViewModel>> GetBlogEntries(Context context, int articleCount = int.MaxValue, bool doHttpRequest = false)
        {
            return Task.Run(async () =>
           {
               var feed = await Subscriber.Subscribe(RssUrl, context, articleCount, doHttpRequest);
               var viewModels = from entries in feed.Entries
                                select new BlogEntryViewModel { AbstractExpanded = false, Entry = entries };
